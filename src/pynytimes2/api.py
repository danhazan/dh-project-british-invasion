...
class NYTAPI:
    ...
    def article_search(self, query=None, dates=None, options=None, count=False):
        ...
        while results - i*10 > 0:
            ...
            results = res.json().get("response").get("meta").get("hits")

            if results >= 2010:
                print("WARNING: TOO MANY PAGES! ({} results)".format(results))
            ...
            yield res.json().get("response").get("docs")
